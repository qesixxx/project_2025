...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadCats() {
        try {
            ...
            displayCats(cats.slice(0, getCatsPerPage()));
            setupFiltering(cats);
    ...
    function setupFiltering(cats) {
        filterButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                const catsPerPage = getCatsPerPage();
                const filter = button.dataset.filter;

                const filteredCats = filter === 'all'
                    ? cats.slice(0, catsPerPage)
                    : cats.filter(cat => cat.category === filter).slice(0, catsPerPage);
